...
- [GitHub Action with Vercel (Recommendation)](#github-action-with-vercel-recommendation)
- [Docker](#docker)
- [Local](#local)
...
env:
  ...
  ATHLETE: Heigao
  TITLE: Heigao Running
  MIN_GRID_DISTANCE: 2 # change min distance here
...
services:
  running_page:
    build:
      context: ./
      dockerfile: Dockerfile         # 使用当前目录作为构建上下文
      args:               # 定义构建参数
        app: Strava
        client_id: ""
        client_secret: ""
    ...
    ports:
      - "8090:80"         # 映射宿主机的8090端口到容器的80端口
    restart: unless-stopped
    tty: true          # 对应 docker run 的 -t 参数
    stdin_open: true   # 对应 docker run 的 -i 参数
```

Supported app and corresponding args.

| App            | Parameter                                                   |
| -------------- | ----------------------------------------------------------- |
| NRC            | nike_refresh_token                                          |
| Garmin         | secret_string                                               |
| Garmin-CN      | secret_string                                               |
| Strava         | client_id, client_secret, refresh_token                     |
| Nike_to_Strava | nike_refresh_token, client_id, client_secret, refresh_token |
| Keep           | keep_phone_number,  keep_password                           |
...
```shell
#!/bin/bash
cd /path/to/your/docker-compose  # 改为你的 docker-compose.yml 所在目录
docker compose down
...
4. Add task, change the path to your `rebuild.sh` path

```shell
0 2 * * * /path/to/rebuild.sh
```
...
You can run the program locally. Similar as docker, you need to fetch the running data manually to keep the running page fresh.
...
sudo npm install -g corepack && corepack enable && pnpm install
```
